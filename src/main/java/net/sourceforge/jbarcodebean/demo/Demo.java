...
class DemoFrame extends JFrame {
    ...
    private void jbInit() throws Exception {
        ...
        pngButton.setToolTipText("Write barcode PNG file");
        pngButton.setText("Output PNG");
    ...
    void pngOutput(ActionEvent e) {
        // Prompt for file name, then output png file.
        if (barcode.getAngleDegrees() % 90 != 0) {
            if (JOptionPane.showConfirmDialog(this, "Generating PNGs for barcodes that are not " +
        ...
        if (f != null) {
            if (!f.getPath().toUpperCase().endsWith(".PNG")) {
                f = new File(f.getPath() + ".PNG");
            }
            try {
                FileOutputStream fos = new FileOutputStream(f);
                BufferedImage img=barcode.draw(null);
                ImageIO.write(img, "png", fos);
                fos.close();
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(this, "Error writing PNG file", "Error", JOptionPane.ERROR_MESSAGE);
            }
        }
    }
